pipeline {
    ...
    environment {
        // GitHub Repository
        GIT_REPO = 'https://github.com/aristit0/Cloud_Inquiry_Balance_Frontend.git'
        GIT_BRANCH = 'main'
        
        // Docker Configuration
        DOCKER_IMAGE = 'cloud-inquiry-balance-frontend'
        DOCKER_TAG = "${BUILD_NUMBER}"
        DOCKER_CONTAINER = 'cloud-inquiry-balance-frontend'
            ...
            steps {
                ...
                script {
                    sh '''
                        ...
                        docker ps | grep cloud-inquiry-balance-frontend
                        
                        echo "\nChecking container logs:"
                        docker logs cloud-inquiry-balance-frontend --tail 20
                        
                        echo "\nTesting health endpoint:"
                        for i in {1..10}; do
                            if curl -f http://localhost:2116/health; then
                                echo "\n✅ Health check passed!"
                                exit 0
                            fi
                        ...
                        curl -I http://localhost:2116/ || echo "⚠️  Main page check failed"
                    '''
                }
            }
        }
